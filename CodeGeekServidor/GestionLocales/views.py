...
def registrar_local (request):
    ...
    if request.method == "POST":
        ...
        nuevo_local.url_img = ruta1.replace('templates/build/static/media/', 'http://www.codegeeks.tk/static/media/')
        ...
        try:
            for i in imagen:
                ...
                ruta=str(img.imagen)
                ruta=ruta.replace('templates/build/static/media/', 'http://www.codegeeks.tk/static/media/')
                img.url = ruta
                img.save()
        except:
